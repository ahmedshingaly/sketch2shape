...
def block_generation(cen_size, color):
    ...
    Colors.InsertNextTuple3(25, 25, 25)
    Colors.InsertNextTuple3(50, 50, 50)
    Colors.InsertNextTuple3(75, 75, 75)
    Colors.InsertNextTuple3(100, 100, 100)
    Colors.InsertNextTuple3(125, 125, 125)
    ...
    cubeActor.GetProperty().SetAmbient(0)
    cubeActor.GetProperty().SetDiffuse(1)
    ...
    for k in range(dims[2]):
        for j in range(dims[1]):
            for i in range(dims[0]):
                ...
                actors.append(
                    block_generation([i + 0.5, j + 0.5, k + 0.5, block_size, block_size, block_size], color=(color)))
                counter = counter + 1

    print
    counter, "blocks filled"
    return actors
